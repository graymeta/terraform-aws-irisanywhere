...
resource "aws_security_group" "alb" {
  name_prefix = replace("${var.hostname_prefix}-${var.deployment_name != "1" ? var.deployment_name : var.instance_type}-alb", ".", "")
  description = replace("${var.hostname_prefix}-${var.deployment_name != "1" ? var.deployment_name : var.instance_type}-alb", ".", "")
...
resource "aws_security_group" "iris" {
  name_prefix = replace("${var.hostname_prefix}-${var.deployment_name != "1" ? var.deployment_name : var.instance_type}-iris", ".", "")
  description = replace("${var.hostname_prefix}-${var.deployment_name != "1" ? var.deployment_name : var.instance_type}-iris", ".", "")
  vpc_id      = data.aws_subnet.subnet.0.vpc_id

  tags = merge(
    local.merged_tags, {
    "Name" = replace("${var.hostname_prefix}-${var.deployment_name != "1" ? var.deployment_name : var.instance_type}-iris", ".", "") }
  )
}
