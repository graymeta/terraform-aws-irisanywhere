resource "aws_lb" "iris_alb" {
  ...
  name_prefix                = substr(replace("${var.hostname_prefix}-${var.instance_name != "1" ? var.instance_name : var.instance_type}-alb", ".", ""), 0, 6)
...
resource "aws_lb_target_group" "port443" {
  count       = var.haproxy ? 0 : 1
  name_prefix = substr(replace("${var.hostname_prefix}-${var.instance_name != "1" ? var.instance_name : var.instance_type}-ScaleIn", ".", ""), 0, 6)
