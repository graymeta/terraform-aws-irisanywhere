...
# resource "aws_cloudwatch_metric_alarm" "missing_data_scalein" {
#   alarm_name = replace(
#     "${var.hostname_prefix}-${var.deployment_name != "1" ? var.deployment_name : var.instance_type}-MissingData-ScaleIn",
#     ".",
#     ""
#   )

#   # Missing => datapoint_count == 0, so alarm when < 1
...
#   alarm_actions     = [aws_autoscaling_policy.in.arn]
#   alarm_description = "Scale in if IrisAvailableSessions has no datapoints for 15 minutes"
...
#   metric_query {
#     id          = "e1"
#     expression  = "TIME_SERIES(DATAPOINT_COUNT(m1))"
#     label       = "DatapointCount(IrisAvailableSessions)"
#     return_data = true
#   }
# }
